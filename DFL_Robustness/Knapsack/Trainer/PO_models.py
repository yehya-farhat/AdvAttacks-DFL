...
from Trainer.utils import batch_solve, regret_fn, abs_regret_fn, regret_list,  growpool_fn, fooling_regret_fn
...
class baseline_mse(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        if len(batch) == 4:
            ...
            y_hat =  self(x).squeeze()
            adv_y_hat = self(x_eps).squeeze()
            
            ### adv regret
            val_loss= regret_fn(self.solver, adv_y_hat,y,sol)
            abs_val_loss= abs_regret_fn(self.solver, adv_y_hat,y,sol)
            
            #### mean accuracy error
            mseloss = criterion(adv_y_hat, y)
            
            ### fooling regret (procudes suboptimal decisions)
            #fooling_val_loss= regret_fn(self.solver, adv_y_hat,y_hat,sol)
            #fooling_abs_val_loss= abs_regret_fn(self.solver, adv_y_hat,y_hat,sol)
            
            
            #### fooling error
            l1regret_fooling_error, l2regret_fooling_error =  fooling_regret_fn(self.solver, adv_y_hat, y_hat,y,sol)
            
            fooling_mseloss = criterion(adv_y_hat,y_hat)
            fooling_l1 = l1_loss(adv_y_hat,y_hat)

            self.log("adv_test_regret", val_loss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("adv_abs_test_regret", abs_val_loss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("adv_test_mse", mseloss, prog_bar=True, on_step=False, on_epoch=True, )
            
            #self.log("fooling_test_regret", fooling_val_loss, prog_bar=True, on_step=False, on_epoch=True, )
            #self.log("fooling_abs_test_regret", fooling_abs_val_loss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_test_mse", fooling_mseloss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_test_l1", fooling_l1, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_regretl2", l2regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_regretl1", l1regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, )
        
            return  {"adv_test_regret": val_loss, "adv_test_mse": mseloss, }
