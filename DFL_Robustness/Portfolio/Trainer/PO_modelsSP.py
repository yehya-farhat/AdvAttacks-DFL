...
from Trainer.utils import  regret_fn, regret_list, abs_regret_fn, growcache, fooling_abs_regret_fn
...
class baseline(pl.LightningModule):
    def __init__(self,net,exact_solver ,cov, gamma, lr=1e-1, l1_weight=1e-5,max_epochs=30, seed=20, scheduler=False, **kwd):
        ...
        #YF required by new lightning module
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #YF required by new lightning module
        self.validation_step_outputs.append({"val_mse":mseloss, "val_regret":regret_loss})
        return {"val_mse":mseloss, "val_regret":regret_loss}
    def on_validation_epoch_end(self):
        avg_regret = torch.stack([x["val_regret"] for x in self.validation_step_outputs]).mean()
    ...
    def test_step(self, batch, batch_idx):
        if len(batch) == 4:
            ...
            adv_x,x,y, sol = batch
            y_hat =  self(x).squeeze()
            adv_y_hat = self(adv_x).squeeze()
            
            
            mseloss = criterion(adv_y_hat, y)
            regret_loss =  regret_fn(self.exact_solver, adv_y_hat,y, sol) 
            abs_regret_loss =  abs_regret_fn(self.exact_solver, adv_y_hat,y, sol) 
            abs_pred = torch.abs( adv_y_hat ).mean()
            
            ####fooling metric
            l1regret_fooling_error, l2regret_fooling_error = fooling_abs_regret_fn(self.exact_solver, adv_y_hat,y_hat,y, sol)
            ...
            self.log("adv_test_regret", regret_loss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("adv_test_abs_regret", abs_regret_loss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("adv_val_absolute_value", abs_pred, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_test_l2", fooling_mseloss, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("fooling_l1", fooling_l1, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("abs_regretfooling_l2", l2regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, )
            self.log("abs_regretfooling_l1", l1regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, )
