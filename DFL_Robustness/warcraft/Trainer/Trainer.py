...
from Trainer.metric import normalized_regret, regret_list, normalized_hamming, fooling_regret
...
class SPO(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        if len(batch) == 4:
            adv_input, input, label, true_weights = batch
            output = self(input)
            adv_output = self(adv_input)
            # output = torch.sigmoid(output)

            if not len(output.shape) == 3:
                output = output.view(label.shape)
            if not len(adv_output.shape) == 3:
                adv_output = adv_output.view(label.shape)
                
            weights = output.reshape(-1, output.shape[-1], output.shape[-1])
            shortest_path =  shortest_pathsolution(self.solver, weights)
    
            adv_weights = adv_output.reshape(-1, adv_output.shape[-1], adv_output.shape[-1])
            adv_shortest_path =  shortest_pathsolution(self.solver, adv_weights)
            ...
            mse =  criterion1(adv_output, true_weights).mean()
            
            ###fooling  error
            fooling_mse = criterion1(adv_output,output)
            fooling_l1 = l1_loss(adv_output,output)
            
            l1regret_fooling_error, l2regret_fooling_error = fooling_regret(true_weights, label, adv_shortest_path, shortest_path)
            
            # if self.loss!= "bce":
            adv_output = torch.sigmoid(adv_output)
            criterion2 = nn.BCELoss()
            bceloss = criterion2(adv_output, label.to(torch.float32)).mean()

            regret = normalized_regret(true_weights, label, adv_shortest_path )   

            Hammingloss = normalized_hamming(true_weights, label, adv_shortest_path ) 

            self.log("adv_test_bce", bceloss, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log("adv_test_mse", mse, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True )
            self.log("adv_test_regret", regret, prog_bar=True, on_step=False, on_epoch=True,sync_dist=True )
            self.log("adv_test_hammingloss",  Hammingloss, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log("fooling_l2", fooling_mse, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log("fooling_l1", fooling_l1, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log("regretfooling_l2", l2regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log("regretfooling_l1", l1regret_fooling_error, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)

            return {"adv_test_mse":mse, "adv_test_bce":bceloss,
                "adv_test_regret":regret,"adv_test_hammingloss":Hammingloss}
